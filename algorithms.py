...
from vis.visual import write_on_image, visualise, activity_dict, visualise_tracking , falling_activities
...
import torch
import math
from message import message, messagewp
    ...
    while True:
        ...
        if not any(q.empty() for q in queues):
            ...
            if argss[0].num_cams == 1:
                ...
                falling_activities = [1.0, 2.0, 3, 4, 5]
                if prediction in falling_activities:
                    # Get the name of the activity that triggered the fall
                    activity = activity_dict[prediction]
                    print("hi")
                    message, messagewp
                ...
                if prediction1 in falling_activities:
                    # Get the name of the activity that triggered the fall
                    activity = activity_dict[prediction1]
                    print("hi")
                    message, messagewp

                if prediction2 in falling_activities:
                    # Get the name of the activity that triggered the fall
                    activity = activity_dict[prediction2]
                    # print("hi")
                    print("hi")
                    message , messagewp
